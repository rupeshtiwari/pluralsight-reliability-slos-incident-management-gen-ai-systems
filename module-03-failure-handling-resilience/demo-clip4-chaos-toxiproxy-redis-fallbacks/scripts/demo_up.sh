...
log "Starting Redis + Toxiproxy + Prometheus + Tempo + Grafana"
docker compose -f infra/docker-compose.yaml up -d --remove-orphans 2>&1 | tail -5

log "Waiting for Redis"
for i in $(seq 1 30); do
  docker exec clip4-redis redis-cli ping 2>/dev/null | grep -q PONG && echo "[OK] Redis :6379" && break
  sleep 1
  if [ "$i" -eq 30 ]; then echo "[ERR] Redis not ready"; exit 1; fi
done

log "Waiting for Toxiproxy admin API"
for i in $(seq 1 30); do
  curl -sf localhost:8474/proxies >/dev/null 2>&1 && echo "[OK] Toxiproxy :8474" && break
  sleep 1
  if [ "$i" -eq 30 ]; then echo "[ERR] Toxiproxy not ready"; exit 1; fi
done
...
log "Starting GenAI chaos service on :8000"
...
log "Warming answer cache with baseline request"
curl -sf -X POST localhost:8000/ask \
  -H 'content-type: application/json' \
  -d '{"question":"What is an SLO?"}' > /dev/null && echo "[OK] Cache warmed for 'What is an SLO?'"
